...
narration_text="You already use a broker, exchange, or market data service. True Fix Studio brings those services into one desktop app. Compare prices and news, and open the original source when something needs checking. Ask the A I assistant to organize information, and test a strategy before using real money. Before sending a trade, check the account, order, fees, and risk. Market Twin is a future idea: compare possible routes and outcomes before a trade. True Fix Studio. Keep your accounts. Make the workflow clearer."

say -v Samantha -r 168 -o "$voice_file" "$narration_text"
...
ffmpeg -hide_banner -loglevel warning -y \
  ...
  -filter_complex "
    [0:v]drawbox=x=130:y=290:w=10:h=340:color=0x74b8ec:t=fill,
      ...
      drawtext=fontfile='${font_mono}':text='RESEARCH AND TRADING SUPPORT':fontcolor=0x74b8ec:fontsize=28:x=198:y=500,
      drawtext=fontfile='${font_mono}':text='CONNECT THE SERVICES YOU ALREADY USE':fontcolor=0x7f8b98:fontsize=21:x=198:y=570,
    ...
    [1:v]fps=30,scale=2140:-2,crop=1920:1080:x='(iw-ow)*t/6':y='(ih-oh)*(1-t/6)',
      ...
      drawtext=fontfile='${font_mono}':text='01 / MARKET AND TRADING':fontcolor=0x74b8ec:fontsize=24:x=110:y=870,
      drawtext=fontfile='${font_regular}':text='See prices, positions, and orders in one place.':fontcolor=white:fontsize=48:x=110:y=930,
    ...
    [2:v]fps=30,scale=2140:-2,crop=1920:1080:x='(iw-ow)*(1-t/6)':y='(ih-oh)*t/6',
      drawbox=x=0:y=820:w=1920:h=260:color=black@0.72:t=fill,
      drawtext=fontfile='${font_mono}':text='02 / RESEARCH AND SOURCES':fontcolor=0x74b8ec:fontsize=24:x=110:y=870,
      drawtext=fontfile='${font_regular}':text='Open the source behind news and signals.':fontcolor=white:fontsize=48:x=110:y=930,
    ...
    [3:v]fps=30,scale=2140:-2,crop=1920:1080:x='(iw-ow)*t/6':y='(ih-oh)*t/6',
      drawbox=x=0:y=820:w=1920:h=260:color=black@0.72:t=fill,
      drawtext=fontfile='${font_mono}':text='03 / AI AND STRATEGY TESTING':fontcolor=0x74b8ec:fontsize=24:x=110:y=870,
      drawtext=fontfile='${font_regular}':text='Ask, test, and check before using real money.':fontcolor=white:fontsize=48:x=110:y=930,
    ...
    [4:v]drawgrid=width=120:height=120:thickness=1:color=white@0.045,
      ...
      drawtext=fontfile='${font_regular}':text='Compare possible outcomes':fontcolor=0x74b8ec:fontsize=54:x=190:y=390,
      drawtext=fontfile='${font_regular}':text='before a trade.':fontcolor=0x74b8ec:fontsize=54:x=190:y=455,
      ...
      drawtext=fontfile='${font_mono}':text='RISK / POSSIBLE OUTCOME':fontcolor=0x9aa8b5:fontsize=18:x=958:y=627,
    ...
    [5:v]drawbox=x=130:y=315:w=10:h=285:color=0x4dc8a0:t=fill,
      drawtext=fontfile='${font_regular}':text='KEEP YOUR ACCOUNTS.':fontcolor=white:fontsize=88:x=190:y=350,
      drawtext=fontfile='${font_regular}':text='MAKE THE WORKFLOW CLEARER.':fontcolor=0x74b8ec:fontsize=78:x=190:y=455,
...
printf '%s\n' "Built $output_file with Samantha narration, an original two-layer ambient score, and the labeled Market Twin future concept"
