...
TrueFix Studio는 시장 조사, 전략 테스트, 거래 전 확인을 위한 데스크톱 앱입니다. 이미 사용 중인 서비스를 연결하지만 자금을 보관하거나 증권사와 거래소를 대신하지 않습니다.

처음에는 공식 버전을 설치하고 워크스페이스를 연 뒤 시세 데이터나 테스트 서비스 하나만 연결하세요. 종목을 검색하고 전송하지 않는 주문을 만들어 전체 흐름을 확인합니다. 실제 계정 전에 읽기 전용이나 모의 환경부터 시작하세요.

고급 섹션에서는 설정 화면의 영어 이름도 사용합니다. **Provider**는 증권사, 거래소, 데이터, AI 서비스이고, **ClientInstance**는 저장한 하나의 연결이며, **Environment**는 테스트용인지 실거래용인지를 뜻합니다.
